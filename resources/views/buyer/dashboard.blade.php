...
{{--
    This is the page a buyer lands on right after logging in.
    It reuses the same sections/design as the guest landing page
    (hero, categories, deals, trending, new arrivals) but:
      - uses partials.navbar-buyer instead of the guest navbar
        (no Login/Create Account — real account/cart/orders/chat icons instead)
      - has no login-gating or login modal, since the buyer is already signed in
      - hrefs marked with a TODO are placeholders until the corresponding
        routes/controllers (cart, wishlist, orders, chat, account, logout) exist
--}}
@extends('layouts.app')
