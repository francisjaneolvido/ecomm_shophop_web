...
<header class="bg-white border-b border-gray-border sticky top-0 z-50">
    <div class="max-w-310 mx-auto px-4 sm:px-6 lg:px-8">
        <div class="h-14 flex items-center gap-6">
            ...
            <a href="{{ route('buyer.dashboard') }}" class="flex items-center gap-2 shrink-0">
                <img src="{{ asset('images/logo.png') }}" alt="ShopHop" class="w-8 h-8 object-contain">
                <div class="leading-none">
                    <span class="block font-bold text-[14px] text-navy">ShopHop</span>
                    <span class="hidden sm:block text-[6px] tracking-wide text-teal-dark mt-1">HOP IN. SHOP MORE.</span>
                </div>
            </a>

            <nav class="hidden xl:flex items-center gap-1 text-[12px] text-navy shrink-0 ml-2">
                <a href="{{ route('buyer.dashboard') }}" class="px-3.5 py-2 rounded-lg bg-teal/15 text-teal-dark font-medium">Home</a>
                <a href="{{ route('buyer.dashboard') }}#categories" class="px-3.5 py-2 rounded-lg hover:bg-gray-bg hover:text-teal-dark transition">Categories</a>
                <a href="{{ route('buyer.dashboard') }}#deals" class="px-3.5 py-2 rounded-lg hover:bg-gray-bg hover:text-teal-dark transition">Deals</a>
            ...
            <div class="ml-auto md:ml-0 flex items-center gap-1.5 shrink-0">
                ...
                {{-- ACCOUNT DROPDOWN --}}
                <div class="relative" data-hover-menu data-account-menu>
                    <button type="button" data-hover-menu-toggle aria-haspopup="true" aria-expanded="false" title="Account"
                            class="flex items-center gap-1.5 h-8 pl-1 pr-2 sm:pr-2.5 rounded-full text-navy hover:bg-gray-bg hover:text-teal-dark transition">
                        <span class="w-6.5 h-6.5 rounded-full bg-teal/15 text-teal-dark flex items-center justify-center text-[11px] font-bold shrink-0">{{ $buyerInitial }}</span>
                        <span class="hidden xl:block text-[11px] font-medium whitespace-nowrap">{{ $buyerName }}</span>
                        <x-lucide-chevron-down class="hidden xl:block w-3.5 h-3.5 transition-transform" />
                    </button>

                    <div data-hover-menu-panel class="hidden absolute right-0 top-full pt-2 z-60 w-48">
                        <div class="rounded-2xl bg-white border border-gray-border shadow-xl shadow-navy/10 py-2">
                            <a href="{{ Route::has('buyer.profile') ? route('buyer.profile') : '#' }}"
                               class="flex items-center gap-2.5 px-4 py-2.5 text-[12px] text-navy hover:bg-gray-bg hover:text-teal-dark transition">
    ...
    hoverMenus.forEach(function (menu) {
        ...
        function openMenu() {
            ...
            hoverMenus.forEach(function (otherMenu) {
                ...
                const otherToggle = otherMenu.querySelector('[data-hover-menu-toggle]');
                const otherPanel = otherMenu.querySelector('[data-hover-menu-panel]');
