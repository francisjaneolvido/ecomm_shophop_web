...
<script>
    (function () {
        ...
        const idPreviewModal = document.getElementById('buyer-id-preview-modal');
        const idPreviewFullImage = document.getElementById('buyer-id-preview-full-image');
        const idPreviewPdfFrame = document.getElementById('buyer-id-preview-pdf-frame');
        const idPreviewFileName = document.getElementById('buyer-id-preview-file-name');

        let lastFocusedElement = null;
        let provincesLoaded = false;
        let closeTimeoutId = null;
        let validIdPreviewUrl = null;
        let validIdPreviewType = null;
        ...
        */
        ...
        function closeValidIdPreview() {
            ...
            if (!idPreviewModal) return;

            idPreviewModal.classList.add('hidden');
            idPreviewModal.classList.remove('flex');
            idPreviewModal.setAttribute('aria-hidden', 'true');

            if (idPreviewFullImage) {
                idPreviewFullImage.classList.add('hidden');
                idPreviewFullImage.removeAttribute('src');
            }

            if (idPreviewPdfFrame) {
                idPreviewPdfFrame.classList.add('hidden');
                idPreviewPdfFrame.removeAttribute('src');
            }
        ...
        function openValidIdPreview() {

            if (!idPreviewModal || !validIdPreviewUrl) return;

            if (idPreviewFileName && fileName) {
                idPreviewFileName.textContent = fileName.textContent || 'Uploaded file';
            }

            if (validIdPreviewType === 'image' && idPreviewFullImage) {
                idPreviewFullImage.src = validIdPreviewUrl;
                idPreviewFullImage.classList.remove('hidden');
            ...
            if (validIdPreviewType === 'pdf' && idPreviewPdfFrame) {
                idPreviewPdfFrame.src = validIdPreviewUrl;
                idPreviewPdfFrame.classList.remove('hidden');
            ...
            idPreviewModal.classList.add('flex');
            idPreviewModal.setAttribute('aria-hidden', 'false');
            document.body.classList.add('overflow-hidden');
        ...
        document.addEventListener('click', function (event) {
            ...
            const previewCloseTrigger = event.target.closest('[data-buyer-id-preview-close]');

            if (previewCloseTrigger && idPreviewModal && idPreviewModal.contains(previewCloseTrigger)) {
        ...
        document.addEventListener('keydown', function (event) {
            ...
            if (idPreviewModal && idPreviewModal.getAttribute('aria-hidden') === 'false') {
                closeValidIdPreview();
                return;
            }
        ...
        */
        ...
        const idPreviewCard = document.getElementById('buyer-id-preview-card');
        const idPreviewImage = document.getElementById('buyer-id-preview-image');
        const idPreviewPdf = document.getElementById('buyer-id-preview-pdf');
        const idPreviewStatus = document.getElementById('buyer-id-preview-status');
        const idPreviewViewButton = document.getElementById('buyer-id-preview-view');
        const idPreviewChangeButton = document.getElementById('buyer-id-preview-change');
        ...
        */
        ...
        function clearValidIdPreview() {
            ...
            validIdPreviewType = null;
            closeValidIdPreview();

            if (idPreviewCard) idPreviewCard.classList.add('hidden');
            ...
            if (idPreviewPdf) {
                idPreviewPdf.classList.add('hidden');
                idPreviewPdf.classList.remove('flex');
            }

            if (idPreviewStatus) idPreviewStatus.textContent = 'Preview ready';

        }

        function renderValidIdPreview(file) {

            clearValidIdPreview();

            validIdPreviewUrl = URL.createObjectURL(file);
            validIdPreviewType = file.type === 'application/pdf' ? 'pdf' : 'image';

            if (idPreviewCard) idPreviewCard.classList.remove('hidden');

            if (validIdPreviewType === 'image' && idPreviewImage) {
                idPreviewImage.src = validIdPreviewUrl;
                ...
                if (idPreviewStatus) idPreviewStatus.textContent = 'Image preview ready';
            }

            if (validIdPreviewType === 'pdf' && idPreviewPdf) {
                idPreviewPdf.classList.remove('hidden');
                idPreviewPdf.classList.add('flex');
                ...
                if (idPreviewStatus) idPreviewStatus.textContent = 'PDF ready to view';
            }

        }

        if (idPreviewViewButton) {
            idPreviewViewButton.addEventListener('click', openValidIdPreview);
        }

        if (idPreviewChangeButton) {
            idPreviewChangeButton.addEventListener('click', function () {
                validIdInput.click();
            });
        }
