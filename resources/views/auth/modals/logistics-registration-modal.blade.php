...
{{-- =========================================================
    WIZARD MOTION + MICRO-INTERACTIONS
    (pure CSS/JS presentation layer — no field names/logic touched)
========================================================= --}}
<style>
    #logistics-wizard-card { will-change: box-shadow; }

    /* Step panel transitions */
    [data-step] { will-change: opacity, transform; }
    .step-anim-fwd { animation: shophopStepFwd .38s cubic-bezier(.22,.68,0,1.01); }
    .step-anim-back { animation: shophopStepBack .38s cubic-bezier(.22,.68,0,1.01); }
    @keyframes shophopStepFwd {
        from { opacity: 0; transform: translateX(18px); }
        to   { opacity: 1; transform: translateX(0); }
    }
    @keyframes shophopStepBack {
        from { opacity: 0; transform: translateX(-18px); }
        to   { opacity: 1; transform: translateX(0); }
    }
    ...
    /* Stepper */
    [data-step-circle] {
        transition: background-color .35s ease, color .35s ease, box-shadow .35s ease, border-color .35s ease, transform .25s ease;
    }
    [data-step-circle].bg-teal { box-shadow: 0 0 0 5px rgba(13,148,136,.14); }
    [data-progress-fill] { transition: width .5s cubic-bezier(.22,.68,0,1.01); }
    [data-step-label] { transition: color .3s ease; }

    /* Smooth focus/hover on form controls inside the wizard */
    #logistics-wizard-card input,
    #logistics-wizard-card select,
    #logistics-wizard-card textarea {
        transition: border-color .2s ease, box-shadow .2s ease, background-color .2s ease;
    }

    /* Terms scroll box — subtle scrollbar + bottom fade hint */
    [data-terms-scroll] { scrollbar-width: thin; scrollbar-color: #99cfc9 transparent; }
    [data-terms-scroll]::-webkit-scrollbar { width: 6px; }
    [data-terms-scroll]::-webkit-scrollbar-thumb { background-color: #99cfc9; border-radius: 999px; }
    [data-terms-scroll-wrap] { position: relative; }
    [data-terms-scroll-wrap]::after {
        content: '';
        position: absolute; left: 1px; right: 7px; bottom: 1px; height: 32px;
        background: linear-gradient(to bottom, transparent, rgba(249,250,251,.95));
        border-radius: 0 0 1rem 1rem;
        pointer-events: none;
        transition: opacity .25s ease;
    }
    [data-terms-scroll-wrap].terms-at-bottom::after { opacity: 0; }

    [data-terms-checkbox]:not(:disabled) { animation: unlockPulse .55s ease; }
    ...
    .otp-box.otp-filled { animation: otpPop .2s ease; }
    @keyframes otpPop {
        0% { transform: scale(1); }
    ...
    .req-dot { transition: background-color .2s ease, border-color .2s ease, transform .2s ease; }
    .req-dot.bg-teal { animation: reqPop .25s ease; }
    @keyframes reqPop {
        0% { transform: scale(.7); }
        60% { transform: scale(1.18); }
        100% { transform: scale(1); }
    }
    [data-password-strength-fill] { transition: width .3s ease, background-color .3s ease; }

    /* Coverage chips fade in as they're added */
    [data-coverage-chip] { animation: chipFadeIn .25s ease; }
    ...
    /* Buttons */
    .shophop-btn-primary { transition: transform .2s ease, box-shadow .2s ease, background-color .2s ease; }
    .shophop-btn-primary:active { transform: translateY(0) scale(.98); }
    .shophop-btn-secondary { transition: background-color .2s ease, color .2s ease, transform .2s ease; }
    .shophop-btn-secondary:active { transform: scale(.98); }
</style>
...
<section class="relative overflow-hidden bg-gray-bg">
    <div class="absolute -top-28 -right-28 w-72 h-72 sm:w-96 sm:h-96 rounded-full bg-teal/10 blur-3xl"></div>
    <div class="absolute -bottom-20 -left-20 w-64 h-64 rounded-full bg-navy/5 blur-3xl hidden sm:block"></div>

    <div class="relative max-w-310 mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 text-center">
        <div class="inline-flex items-center gap-2 bg-teal-light text-teal-dark px-3.5 sm:px-4 py-2 rounded-full text-xs sm:text-sm font-semibold mb-5 sm:mb-6 shadow-sm shadow-teal/10">
            <span class="w-2 h-2 rounded-full bg-teal"></span>
            Partner with ShopHop
        </div>

        <h1 class="text-navy font-bold tracking-tight mb-4">
...
{{-- =========================================================
    APPLICATION FORM
========================================================= --}}
<section class="py-12 sm:py-16 bg-gray-bg">
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">

        @if (session('status'))
            <div class="flex items-center gap-3 bg-teal-light text-teal-dark text-sm font-medium px-4 py-3 rounded-xl mb-6 shadow-sm">
                <x-lucide-check-circle-2 class="w-4 h-4 shrink-0" />
                {{ session('status') }}
            </div>
        @endif

        <div id="logistics-wizard-card" class="bg-white border border-gray-border rounded-3xl shadow-sm shadow-navy/5 p-5 sm:p-8">

        <form method="POST" action="{{ route('logistics.register.store') }}" enctype="multipart/form-data" data-wizard>
            @csrf

            {{-- Step tracker — single animated progress track --}}
            <div class="mb-10">
                <div class="relative">
                    <div class="absolute top-4 left-4 right-4 h-0.5 bg-gray-border rounded-full"></div>
                    <div data-progress-fill class="absolute top-4 left-4 h-0.5 bg-teal rounded-full" style="width:0%"></div>
                        ...
                        @foreach (['Terms & Agreement', 'Company Details', 'Verify Email', 'Enter Code', 'Create Password', 'Coverage & Documents', 'Review & Submit'] as $i => $label)
                            <div class="flex flex-col items-center">
                                <div data-step-circle="{{ $i + 1 }}"
                                     class="relative z-10 w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold {{ $i === 0 ? 'bg-teal text-white' : 'bg-gray-bg text-navy/40 border border-gray-border' }}">
                                    {{ $i + 1 }}
                                </div>
                                <span data-step-label="{{ $i + 1 }}"
                                      class="hidden sm:block text-[10px] lg:text-[11px] font-semibold text-center max-w-16 lg:max-w-20 mt-2 {{ $i === 0 ? 'text-navy' : 'text-navy/40' }}">
                                    {{ $label }}
                                </span>
                            </div>
                        @endforeach
                ...
                {{-- Mobile: just show the current step name + position, instead of 7 cramped labels --}}
                <p class="sm:hidden text-center text-xs font-semibold text-navy mt-3">
                    <span data-step-current-label>Terms &amp; Agreement</span>
                    <span class="text-navy/35 font-medium"> · Step <span data-step-current-number>1</span> of {{ count(['Terms & Agreement', 'Company Details', 'Verify Email', 'Enter Code', 'Create Password', 'Coverage & Documents', 'Review & Submit']) }}</span>
                </p>
            </div>
            ...
            <div data-step="1">
                ...
                <div class="flex items-center justify-between mb-2">
                    ...
                    <a href="{{ route('logistics.terms') }}" target="_blank" rel="noopener"
                       class="text-xs font-semibold text-teal-dark hover:text-teal underline underline-offset-2 flex items-center gap-1 transition-colors">
                        See full terms
                        <x-lucide-external-link class="w-3 h-3" />
                    </a>
                </div>

                <div data-terms-scroll-wrap>
                ...
                <div class="bg-gray-bg rounded-2xl p-5 sm:p-6 mt-5 space-y-5">
                    ...
                    <div>
                        ...
                        <label class="flex items-center gap-2 border border-dashed border-gray-border rounded-xl px-4 py-3 text-sm text-navy/50 cursor-pointer hover:border-teal/40 hover:text-navy/70 hover:bg-teal-light/20 transition bg-white">
            ...
            <div data-step="2" class="hidden grid sm:grid-cols-2 gap-5">
                ...
                <div class="sm:col-span-2">
                    ...
                    <label class="flex items-center gap-2 border border-dashed border-gray-border rounded-xl px-4 py-3 text-sm text-navy/50 cursor-pointer hover:border-teal/40 hover:text-navy/70 hover:bg-teal-light/20 transition">
            ...
            <div data-step="5" class="hidden">
                ...
                <div class="grid sm:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        {{-- Password strength bar (visual aid only, does not change validation rules) --}}
                        <div class="flex items-center gap-1.5 mt-2" data-password-strength>
                            <div class="h-1 flex-1 rounded-full bg-gray-border overflow-hidden">
                                <div data-password-strength-fill class="h-full rounded-full bg-gray-border" style="width:0%"></div>
                            </div>
                            <span data-password-strength-label class="text-[10px] font-semibold text-navy/35 w-12 text-right"></span>
                        </div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <div class="flex flex-col sm:flex-row gap-2">
                        ...
                        <button type="button" data-coverage-add-btn
                                class="shophop-btn-secondary shrink-0 bg-navy text-white text-sm font-semibold px-5 py-3 rounded-xl hover:bg-navy/90 transition">
                            + Add province
                        </button>
                    </div>
            ...
            <div data-step="7" class="hidden">
                <div class="bg-gray-bg rounded-2xl p-6 divide-y divide-gray-border text-sm">
                    ...
                    <div class="py-2.5 flex justify-between gap-4">
                        ...
                        <span class="font-semibold text-navy text-right" data-review-signature-file>—</span>
                    </div>
                </div>

                <label class="flex items-start gap-2.5 mt-6 text-xs text-navy/65 cursor-pointer">
                    <input type="checkbox" required class="mt-0.5 accent-teal rounded">
            ...
            <div class="flex justify-between gap-3 mt-8">
                <button type="button" data-wizard-back
                        class="shophop-btn-secondary hidden border-2 border-navy text-navy hover:bg-navy hover:text-white text-sm font-semibold px-6 py-3 rounded-full transition-all duration-300">
                    Back
                </button>
                <div class="flex-1"></div>
                <button type="button" data-wizard-next
                        class="shophop-btn-primary bg-teal hover:bg-teal-dark text-white text-sm font-semibold px-7 py-3 rounded-full transition-all duration-300 hover:-translate-y-0.5 shadow-lg shadow-teal/20">
                    Continue
                </button>
                <button type="submit" data-wizard-submit
                        class="shophop-btn-primary hidden bg-teal hover:bg-teal-dark text-white text-sm font-semibold px-7 py-3 rounded-full transition-all duration-300 hover:-translate-y-0.5 shadow-lg shadow-teal/20">
                    Submit application
                </button>
            </div>
        </form>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function renderCoverageList() {
            ...
            coverageChips.forEach(function (chip) {
                ...
                wrap.className = 'border border-gray-border rounded-xl p-4 bg-white hover:border-teal/30 transition-colors';
        ...
        function updatePasswordRequirements(value) {
            ...
            // Visual-only strength meter — does not affect validation.
            const strengthFill = wizard.querySelector('[data-password-strength-fill]');
            const strengthLabel = wizard.querySelector('[data-password-strength-label]');
            if (strengthFill && strengthLabel) {
                ...
                strengthFill.className = 'h-full rounded-full ' + tier.color;
        ...
        const termsScrollWrap = wizard.querySelector('[data-terms-scroll-wrap]');
        ...
        function render() {
            steps.forEach(function (panel) {
                const isTarget = Number(panel.dataset.step) === current;
                panel.classList.remove('step-anim-fwd', 'step-anim-back');
                panel.classList.toggle('hidden', !isTarget);
                if (isTarget) {
                    // restart the animation on every step change
                    void panel.offsetWidth;
                    panel.classList.add(lastDirection === 'back' ? 'step-anim-back' : 'step-anim-fwd');
                }
            });
            ...
            // Animated progress fill + mobile current-step label
            const fill = wizard.querySelector('[data-progress-fill]');
            if (fill) fill.style.width = (total > 1 ? ((current - 1) / (total - 1)) * 100 : 0) + '%';

            const currentLabelEl = wizard.querySelector('[data-step-current-label]');
            const currentNumberEl = wizard.querySelector('[data-step-current-number]');
            if (currentLabelEl) currentLabelEl.textContent = stepLabels[current - 1] || '';
            if (currentNumberEl) currentNumberEl.textContent = current;
